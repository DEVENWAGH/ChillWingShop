const initColorSelector = () => {
  ...
  if (black) black.addEventListener("click", () => handleColorChange(info[0].src));
  if (silver) silver.addEventListener("click", () => handleColorChange(info[1].src));
  if (pink) pink.addEventListener("click", () => handleColorChange(info[2].src));
